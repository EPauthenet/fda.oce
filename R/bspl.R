...
#' @param irreg default is 0 for irregular vertical sampling and regular horizontal sampling (model type e.g. SOSE, GLORYS,...). \code{irreg=1} for irregular vertical and horizontal sampling (e.g. \code{DATA_ncARGO} for MEOP). \code{irreg=2} for irregular horizontal sampling but interpolated on a thin vertical grid (1 dbar by level).(e.g. \code{DATA_ncARGO_interp} from MEOP).
...
#' @export
bspl <- function(temp,sal,Depth,range,mybn = 20,l = 100,irreg = 0){
  ...
  }else if (irreg == 1){         ##############For irregular horizontal and vertical sampling
    k     <- 1
    t     <- temp[,k]
    s     <- sal[,k]
  ...
  }else if (irreg == 2){         ##############For irregular horizontal sampling and interpolated data on a thin grid
    k     <- 1
    t     <- temp[,k]
    s     <- sal[,k]
    profx <- Depth[dmin:dmax]
